...
public class SoundGenerator {
    ...
    public static void playSineWave(double frequency, int durationMs) {
        try {
            // Paramètres du son
            float sampleRate = 44100; // Échantillonnage resources.Pictures.audio standard (44.1 kHz)
            ...
            // Configurer le format resources.Pictures.audio
            AudioFormat audioFormat = new AudioFormat(sampleRate, 8, 1, true, true);
            DataLine.Info info = new DataLine.Info(SourceDataLine.class, audioFormat);

            // Ouvrir et démarrer la ligne resources.Pictures.audio
            SourceDataLine line = (SourceDataLine) AudioSystem.getLine(info);
            line.open(audioFormat);
            line.start();

            // Écrire les données resources.Pictures.audio dans la ligne (lecture du son)
            line.write(buffer, 0, buffer.length);

            // Finir et fermer la ligne resources.Pictures.audio
            line.drain();
            line.close();
